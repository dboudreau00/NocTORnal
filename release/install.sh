...
# Rotating either value invalidates what it protects: the TOTP KEK makes
...
if [[ "$USERS" == "0" ]]; then
  ...
  detail 'You will be asked for an email and a password, and shown a QR'
  detail 'code to scan into an authenticator app.'
  printf '\n'
  ( cd "$REPO_ROOT" && "$VENV_PY" scripts/bootstrap.py init )
else
