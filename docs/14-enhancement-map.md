...
| **E4** | Recovery codes: ten single-use Argon2id-hashed codes, per docs/05 | **Built,** and accepted in the sign-in field, which is what makes them a real escape hatch rather than a stored secret nobody can spend. Issued by the console since 2026-09-22: at the first administrator's first sign-in and from Account (the count is shown there); before that only `bootstrap.py recovery-codes` could issue them |
