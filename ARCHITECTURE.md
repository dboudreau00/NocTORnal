...
> **Alembic head 0061** and **1813 tests** (`def test_` functions across both
...
| Evidence store | MinIO (`quay.io/minio/minio`, pinned) + `quay.io/minio/mc` init | WORM evidence via object lock; buckets `noctornal-evidence` (`--with-lock`), `noctornal-raw` (ingest raw bytes, `rawstore.py`) and `noctornal-samples` | Implemented. Compose sets `--default GOVERNANCE 365d`; app writes COMPLIANCE-mode locks (`docs/00` #26) |
